    ...
    <div style={{ padding: '28px 32px', overflowY: 'auto', height: '100%' }}>
      ...
      <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.4px' }}>Dashboard</h1>
        </div>
        {sessions.length > 0 && (
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <select value={activeSessionId ?? ''} onChange={e => onSelectSession(e.target.value)} style={{
                ...
                padding: '7px 28px 7px 11px', fontFamily: 'var(--font-sans)', cursor: 'pointer', minWidth: 180,
              }}>
      ...
      {/* KPIs */}
      <div className="fade-up d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={c.label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '16px 18px' }}>
            <p style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{c.label}</p>
            <span style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-1px', color: c.color || 'var(--white)' }}>{c.value}</span>
          </div>
        ))}
      </div>

      {/* Cards por setor */}
      <div className="fade-up d2" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
        {SETORES.map(setor => {
          ...
          return (
            <div key={setor.label} style={{ background: 'var(--surface)', border: `1px solid ${setor.border}`, borderRadius: 'var(--radius)', padding: '16px 18px' }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: setor.color, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>{setor.label}</p>
              <p style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-1px', marginBottom: 10 }}>{sl.length}</p>
              <div style={{ display: 'flex', gap: 12, fontSize: 11, color: 'var(--text-secondary)' }}>
                <span><Dot color="var(--teal)" />Alto: {h}</span>
                <span><Dot color="#f59e0b" />Médio: {m}</span>
                <span><Dot color="var(--text-muted)" />Baixo: {lo}</span>
              </div>
            </div>
          )
        })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        {/* Score breakdown */}
        <div className="fade-up d2" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '18px 20px' }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>Distribuição de fit</p>
          {[
        ...
        {/* Pipeline */}
        <div className="fade-up d2" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '18px 20px' }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>Pipeline</p>
          {byStatus.length === 0
      ...
      {/* FUP */}
      {fupAlerts.length > 0 && (
        <div className="fade-up d3" style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.18)', borderRadius: 'var(--radius)', padding: '14px 18px', marginBottom: 16 }}>
      ...
      {/* Top leads */}
      {topLeads.length > 0 && (
        <div className="fade-up d4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '18px 20px' }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>Leads de maior potencial</p>
          {topLeads.map(lead => (
